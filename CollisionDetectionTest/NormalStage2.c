...
Brief Description:
Contents of Normal Stage 2 object of the game.
******************************************************************************/
/* Load Libraries */
#include "stdafx.h"
...
#include "NormalStage2.h"		/* function declaration */
#include "StageData.h"			/* common functions of stages */
#include "IceStage1.h"			/* iceStage_1() */
...
/* Normal Terrain Stage 2 */
void normalStage_2(Sound *sound, int play_type, int terrain)
{
	/* colour */
	...
	normalStage = Stage_Create(play_type, 1, NUM_ENEMIES, NUM_ROWS, SIZE_X, SIZE_Y, xBorder, yBorder, HORIZONTAL_GAP, VERTICAL_GAP, 
								 2, 8, 3, 2, 0, 0, 0, 0, PLAYER1_START_POSY, PLAYER2_START_POSY, terrain);
	...
	/* intiialize the normalStage current_time */
		...
		*************************/
		...
		if (_kbhit())
		{
			...
			if (input == 'p')
			{
				...
				optionmenu_Print(normalStage, 2, 0);
				select = _getch();
	...
	***********/
	...
	if (normalStage->play_type == CoOp && CoOp_Enemy_Check(normalStage, enemyContainer_P1, enemyContainer_P2))
	{
		...
		position = Highscore_Check(player_1, player_2, 2);
		system("cls");
		normalStage->size_x = 80;
		normalStage->size_y = 25;
		/* display the highscore to player */
		Highscore_Display_Stage(normalStage, 2, position);
		/* free stage */
		Stages_Free(normalStage, player_1, player_2, enemyContainer_P1, enemyContainer_P2, enemyBullet, enemy_AI_P1, enemy_AI_P2,
			powerup, ball_1, ball_2, tile_manager);
		...
		/* Play ice stage 1 background music */
