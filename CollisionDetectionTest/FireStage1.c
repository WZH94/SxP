...
Brief Description:
Contents of Fire Stage 1 object of the game.
******************************************************************************/
/* Load Libraries */
#include "stdafx.h"
...
#include "FireStage1.h"			/* function declaration */
#include "FireStage2.h"			/* firestage_2() */
...
void fireStage_1(Sound *sound, int play_type, int terrain)
{
	...
	fireStage = Stage_Create(play_type, 1, NUM_ENEMIES, NUM_ROWS, SIZE_X, SIZE_Y, xBorder, yBorder, HORIZONTAL_GAP, VERTICAL_GAP, 
							 5, 0, 2, 1, 2, 2, 2, 7, PLAYER1_START_POSY, PLAYER2_START_POSY, terrain);
	...
	while (1)
	{
		...
		*************************/
		...
		Enemy_Fire(fireStage, enemyContainer_P1, enemyBullet, sound); /* for P1 */
		Enemy_Fire(fireStage, enemyContainer_P2, enemyBullet, sound); /* for P2 */
	...
	***********/
	...
	if (fireStage->play_type == CoOp && CoOp_Enemy_Check(fireStage, enemyContainer_P1, enemyContainer_P2))
	{
		...
		printf("YOU HAVE WON! MOVE ON TO THE FINAL ROUND!");
		...
		position = Highscore_Check(player_1, player_2, 5);
		system("cls");
		fireStage->size_x = 80;
		fireStage->size_y = 25;
		/* display the highscore to player */
		Highscore_Display_Stage(fireStage, 5, position);
		/* free stage */
		Stages_Free(fireStage, player_1, player_2, enemyContainer_P1, enemyContainer_P2, enemyBullet, enemy_AI_P1, enemy_AI_P2,
			powerup, ball_1, ball_2, tile_manager);
		...
		/* Play fire stage 2 background music */
