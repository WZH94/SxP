...
Brief Description:
Contents of Ice Stage 1 object of the game.
******************************************************************************/
/* Load Libraries */
#include "stdafx.h"
...
#include "IceStage1.h"			/* function declaration */
#include "IceStage2.h"			/* icestage_2() */
...
void iceStage_1(Sound *sound, int play_type, int terrain)
{
	...
	iceStage = Stage_Create(play_type, 1, NUM_ENEMIES, NUM_ROWS, SIZE_X, SIZE_Y, xBorder, yBorder, HORIZONTAL_GAP, VERTICAL_GAP, 
							3, 0, 6, 6, 1, 2, 0, 0, PLAYER1_START_POSY, PLAYER2_START_POSY, terrain);
	...
	while (1)
	{
		...
		*************************/
		...
		Enemy_Fire(iceStage, enemyContainer_P1, enemyBullet, sound);
		Enemy_Fire(iceStage, enemyContainer_P2, enemyBullet, sound);
	...
	***********/
	...
	if (iceStage->play_type == CoOp && CoOp_Enemy_Check(iceStage, enemyContainer_P1, enemyContainer_P2))
	{
		...
		position = Highscore_Check(player_1, player_2, 3);
		system("cls");
		iceStage->size_x = 80;
		iceStage->size_y = 25;
		/* display the highscore to player */
		Highscore_Display_Stage(iceStage, 3, position);
		/* free stage */
		Stages_Free(iceStage, player_1, player_2, enemyContainer_P1, enemyContainer_P2, enemyBullet, enemy_AI_P1, enemy_AI_P2,
			powerup, ball_1, ball_2, tile_manager);
